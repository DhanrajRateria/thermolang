...
from generate_barbell import generate_barbell
...
def main():
    ...
    src = generate_barbell() # Generates examples/barbell.thermo
    
    # 1. Baseline
    e_off, v_off = run_compile_and_sim(src, "off")
    results["barbell"]["off"] = e_off
    
    # 2. Degree (Will fail to identify the bridge as important because it has low degree)
    e_deg, _ = run_compile_and_sim(src, "degree")
    results["barbell"]["degree"] = e_deg
    
    # 3. Variance (Will see the bridge flickering and cool it down)
    if v_off:
        e_var, _ = run_compile_and_sim(src, "degree+variance", variance_data=v_off)
        results["barbell"]["iterative_var"] = e_var
